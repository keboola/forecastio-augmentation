...
class Forecast
{
	 ...
	 *
	 * @throws Exception
	 * @return array|Response|bool If array passed in, returns array of
	...
	public function getData($args1)
	{
		...
		*/
		$requests = array(); // will hold array of arrays of lat/long/time/options
		...
		foreach ($json_results as $result) {
			if ($result !== false) {
				$conditions[] = new Response($result);
			} else {
				$conditions[] = false;
