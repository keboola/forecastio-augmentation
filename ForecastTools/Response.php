...
class Response
{
	...
	/**
	 * Get number of DataPoint objects that exist within specified block
	 *
	 * @param string $type Type of data block
	 *
	 * @return int Returns number of DataPoint objects that exist within
	 * specified block
	 */
	...
	/**
	 * Get DataPoint object for current or specified time
	 *
	 * @return DataPoint DataPoint object for current or specified time
	 */
	public function getCurrently()
	{
		return new DataPoint($this->response->currently);
	}

	/**
	 * Get DataPoint object(s) desired within the specified block
	 *
	 * @param string $type  Type of data block (
	 * @param int    $index Optional numeric index of desired data point in block
	 * beginning with 0
	 *
	 * @return array|DataPoint|bool Returns an array of DataPoint
	 * objects within the block OR a single DataPoint object for specified
		...
		if ($this->getCount($type)) {
			...
			if (is_null($index)) {
				...
				foreach ($block_data as $point_data) {
					$points[] = new DataPoint($point_data);
				}
				return $points;
			} elseif (is_int($index) && $this->getCount($type) > $index) {
				return new DataPoint($block_data[$index]);
			}
	...
	/**
	 * Get DataPoint object(s) desired within the minutely block, which is
	...
	/**
	 * Get DataPoint object(s) desired within the hourly block, which is
	...
	/**
	 * Get DataPoint object(s) desired within the daily block, which is
	...
	public function getAlerts()
	{
		...
		if (!empty($this->response->alert)) {
			$alerts = array();
			foreach ($this->response->alert as $alert) {
	 ...
	 *
	 * @return Flags|bool ForecastFlags object OR false if none
	 */
	public function getFlags()
	{

		if (!empty($this->response->flags)) {
			return new Flags($this->response->flags);
		} else {
			return false;
		}
